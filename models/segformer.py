...
class Segformer(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            self.load_checkpoint(self, pretrained, map_location='cpu', strict=False, logger=None)
    ...
    def forward(self, x):
       ...
       ############## MLP decoder on C1-C4 ###########
        ...
        _c_pre = torch.cat([_c4, _c3, _c2, _c1], dim = 1)
        _c = self.linear_fuse(_c_pre)

        x = self.dropout(_c)
        x = self.linear_pred(x)
    
        return [x, _c_pre ,c, _c]
