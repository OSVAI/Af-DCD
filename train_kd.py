...
def parse_args():
    ...
    parser.add_argument("--lambda-dmgd", type=float, default=0., help="lambda mgd")
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_dmgd = DenseNormFeatureLoss().to(self.device)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            if self.args.lambda_dmgd != 0.:
                dmgd_loss = self.args.lambda_dmgd * self.criterion_dmgd(s_outputs[-1], t_outputs[-1])


            losses = task_loss + kd_loss + adv_G_loss + \
                        skd_loss + cwd_fea_loss + cwd_logit_loss +\
                        ifv_loss + at_loss + fitnet_loss +\
                        psd_loss + csd_loss + dmgd_loss
            D_losses = adv_D_loss
            ...
            dmgd_loss_reduced = self.reduce_mean_tensor(dmgd_loss)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    ...
                    "|| skd_loss: {:.4f} || cwd_fea_loss: {:.4f} || cwd_logit_loss: {:.4f} " \
                        ...
                        "|| psd_loss: {:.4f} || csd_loss: {:.4f} || dmgd_loss: {:.4f}" \
